...
So I tried to do the simulation again on the vivado 2022, and it is giving me the same results as in 2015.

Will have to look close to see why this is happening.

Seems that it is still giving me trouble, I will try another way of implementing state machine to see if it will make any difference.

Tried the state machine with only one register for state.

It is giving me the same simulation results.

But when I space apart the trans_start signal and the posedge clk, it is working perfectly fine. I think it is the metastability issue here.

Will keep the code like this for now and move on the more top level implementation

But I will check other state machine's simulation and see if this exists in my other codes.

In general I think my methodology should be correct.

Because when I connected both layers for simulation, it has not shown any sign of issue between these two layers, and the second layer is triggered by the flag, which is generated by the first layer. No jumping state phenomenon was observed. It is doing the correct behaviour as expected.

Guess it is simply the testbench that I have to be careful with, I should space apart the input and flag from posedge clk. The entire methodology of design is correct.

Think this is simply just he behaviour of vivado simulator, since Xcelium does not do that.

Will see if the simulation will run correctly even for the diagonal 0.

Yes it is correct even for diagonal 0 design, all I need is to space apart the input with the posedge clk.

Start developing the top state machine now.

Just finished the top state machine now, will start testing the SM.

From one test bench drafted, it seems that it is working well.
